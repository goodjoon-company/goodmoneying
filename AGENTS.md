...
| Task 요약(Task Summaries)           | `docs/Task/`              | Milestone별 완료 작업 핵심 요약                |
...
- 임시 진행 메모는 GitHub Issue, PR(Pull Request), 또는 `docs/History/`에 둔다. Product 또는 Architecture 문서에 구현 로그를 늘리지 않는다.
...
- 브레인스토밍 설계 근거는 GitHub Issue, 관련 단일 기준(source of truth) 문서, `docs/ADR/`, 또는 `docs/History/`에 저장한다.
- 구현 계획(implementation plan)은 GitHub Issue로 저장한다.
- 검증 증적(verification evidence)은 `docs/Test/`에 저장한다.
- 인계(handoff)와 오래 유지되는 변경 요약은 `docs/History/`에 저장한다.
- `superpowers` skill이 기본 경로에 설계(design), 명세(spec), 계획(plan) 문서 작성을 지시하면, 해당 내용을 GitHub Issue, 단일 기준(source of truth) 문서, `docs/ADR/`, `docs/Test/`, `docs/History/` 중 맞는 위치로 재지정한다.
- 기존 `superpowers` 산출물이 있으면 유효한 내용을 GitHub Issue, `docs/Test/`, `docs/History/`, 제품(Product), 아키텍처(Architecture), 계약(Contracts), ADR(Architecture Decision Record) 중 맞는 곳으로 이관한 뒤 병렬 산출물을 제거한다.
