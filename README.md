...
goodmoneying은 업비트(Upbit) 가상자산을 대상으로 데이터 수집, 연구, 전략, 백테스트, 모의·실거래 준비를 한 흐름으로 제공하는 개인용 시스템 트레이딩 플랫폼이다. 현재 P1 데이터 기반(Data Foundation)은 전체 시장 시점 이력, 모든 원화(KRW) 시장의 2024-01-01 UTC 자동 정책, 내구성 백필(Backfill), 동적 실시간 구독과 5단계 커버리지 품질을 제공한다. 주식시장 기능은 현재 범위가 아니다.
...
infra는 Podman Compose로 PostgreSQL을 관리하고, app은 로컬 개발 프로세스로 API, web, 업비트 API 게이트웨이(Upbit API Gateway), 시장 동기화 워커, 실시간 수집 워커, 백필 수집 워커, 캔들 집계 워커를 개별 start/stop/status 할 수 있다.
...
./dev.sh app start api
./dev.sh app start upbit-gateway
./dev.sh app start market-sync-worker
./dev.sh app stop api
...
`Coverage & Quality` 화면은 공식 KRW 시장, 자동 수집 대상, 대기 백필, 희망 실시간 구독과 `observed`, `no_trade`, `unavailable`, `unverified`, `failed` 구간을 함께 보여준다. 시장별로 UTC 시작일, 수집 데이터 유형, 1분 기준 주기, 보존 기간, 우선순위, 연속 수집과 활성·일시정지·제외 상태를 설정하면 별도 시작 버튼 없이 DB 정책, 자동 백필과 실시간 구독 세대에 반영된다. 내부 저장·계산은 UTC이고 화면에서 필요한 시각만 KST로 명시한다.

시장을 제외해도 과거 원천과 상태 이력은 삭제하지 않는다. 과거에 수집하지 않은 호가·티커·체결은 합성하지 않고 `unavailable`, 아직 증거를 확정하지 않은 캔들은 `unverified`로 표시한다. 빈 API 응답 하나만으로 `no_trade`를 만들지 않는다.
