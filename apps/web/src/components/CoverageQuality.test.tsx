...
describe("Coverage & Quality", () => {
  it("UTC 기본 정책과 5가지 커버리지 상태를 텍스트로 구분한다", async () => {
    renderCoverage();
  ...
  it("시장별 시작일·데이터 유형·보존·우선순위 정책을 저장한다", async () => {
    ...
    await user.clear(screen.getByLabelText("수집 시작 UTC"));
    await user.type(screen.getByLabelText("수집 시작 UTC"), "2024-02-01T00:00");
