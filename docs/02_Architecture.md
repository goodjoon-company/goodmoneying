...
| 캔들 집계 워커(Candle Aggregation Worker) | 원천 워터마크와 집계 워터마크를 비교해 `5m/10m/30m/60m/1d/1w/1M` OHLCV를 생성한다. | 기본 5초 폴링, 대상별 멱등 upsert와 작업 수명 동안 별도 저장소 연결의 5초 주기 하트비트(heartbeat)를 기록한다. 하트비트 I/O(Input/Output)는 2초 제한, 종료 합류(join)는 3초 유예를 적용한다. |
