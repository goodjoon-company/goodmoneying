...
| 런타임 | 책임 | M1 구현 | 확장 방향 |
|---|---|---|---|
| 실시간 수집 워커(Realtime Collection Worker) | 후보 유니버스(Candidate Universe), 현재가 스냅샷(Ticker Snapshot), 호가 요약(Orderbook Summary), 1분 원천 캔들(Source Candle) 증분 수집(Incremental Collection), 수집 품질 기록 | Python 단일 프로세스, 1분 주기 목표 | M3.5에서 다중 워커와 메시지 큐(Message Queue) 기반 작업 분배 검토 |
| 백필 수집 워커(Backfill Collection Worker) | 승인된 백필 작업(Backfill Job)을 DB 상태 폴링(Polling)으로 확인하고 원천 캔들 백필 수행 | Python 단일 프로세스, 기본 10초 폴링, 동시성(Concurrency) 1 | M3.5에서 코인별 병렬 백필, 분산 rate limiter, 메시지 큐 검토 |
...
| 운영 화면 | 데이터 수집관리 내비게이션, 운영 상태 대시보드, 수집 대상/설정, 시장 리스트, 코인 상세 레이어, 백필 작업 화면 | React, HTTP 폴링(Polling), React Query | 후속 실시간성 화면에서 SSE(Server-Sent Events) 또는 WebSocket 결정 |
...
3. 운영 서버는 선택 코인, 데이터 유형, 목표 기간을 기준으로 백필 계획(Backfill Plan)을 생성하고 예상 요청 수와 저장 예상량을 반환한다.
4. 운영 화면은 생성된 백필 계획들을 백필 승인 패널에 목록으로 구성한다.
5. 사용자가 백필 계획 승인 버튼을 누르면 운영 서버가 계획별 백필 작업(Backfill Job)을 생성한다.
6. 백필 수집 워커는 DB 상태 폴링(Polling)으로 승인된 백필 작업 상태를 10초 주기로 읽고 실행한다.
7. 백필 수집 워커는 이미 저장된 구간을 중복 요청하지 않고, 시작일에 해당하는 데이터가 있으면 그 이후 첫 빈 구간부터 요청한다.
8. 사용자는 실행 중인 백필 작업을 일시정지(Pause), 중지(Stop), 이어서하기(Resume), 안전 재시작할 수 있다.
9. 안전 재시작은 기존 데이터를 삭제하지 않고 목표 범위 전체를 재검사한다.
10. 삭제 후 재수집(Destructive Rebuild)은 M1 이후 기능으로 둔다.
