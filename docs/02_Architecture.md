...
| 운영 화면 | 데이터 수집관리 내비게이션, 운영 상태 대시보드, Backfill 관리, 시장 리스트, 코인 상세 레이어, 백필 작업 화면 | React, 운영 상태는 SSE(Server-Sent Events) 기반 갱신 + React Query HTTP 폴링(Polling) 보조 | 후속 고빈도 화면에서 패널별 증분 이벤트와 재연결 복구 정책 확장 |
