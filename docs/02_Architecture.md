...
M1은 수집 워커(Collection Worker), 운영 서버(Operations Server), 운영 화면, PostgreSQL 저장소로 구성한다. 수집 워커는 업비트 API에서 데이터를 가져와 PostgreSQL에 원천 사실을 저장하고, 운영 서버는 API와 화면용 상태 계산을 제공한다. 운영 화면은 React 기반으로 데이터 수집관리 좌측 내비게이션(Navigation), 운영 상태, 수집 대상, 시장 리스트, 코인 상세, 백필(Backfill) 상태를 보여준다.
...
7. 수집 워커 또는 배치 작업은 코인별 수집 계획, 데이터별 최신성, 결측 구간, 구간형 진행 상태를 계산해 저장된 View Model을 갱신한다.
...
6. 수집 워커는 DB 상태 폴링(Polling)으로 승인된 백필 작업 상태를 읽고 실행한다.
7. 워커는 이미 저장된 구간을 중복 요청하지 않고, 시작일에 해당하는 데이터가 있으면 그 이후 첫 빈 구간부터 요청한다.
