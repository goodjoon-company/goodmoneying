...
| 전략·백테스트·포트폴리오·봇·주문·위험 | P3 전략, P4 백테스트, P5-1 portfolio/bot/order intent/risk DB 계약, P5-2 portfolio API/Store 구현. paper/shadow 실행 연결은 후속 P5 조각 | Issue #30~#33 |
...
| Paper/Shadow Worker | 모의 체결 또는 실시간 신호 관찰, 실제 주문 금지 | PostgreSQL | P5-1 영속화 계약과 P5-2 portfolio Store 구현, worker 미구현 |
...
| Risk Worker | 사전 주문·실시간 노출 검사, 위험 이벤트, 긴급 정지 | PostgreSQL | P5-1 영속화 계약 구현, P5-2 portfolio API와 연결, worker 미구현 |
