...
| 캔들 집계 워커(Candle Aggregation Worker) | 원천봉보다 오래된 집계 단위를 감지하고 작업 대상별 OHLCV rollup을 upsert, 진행률과 처리량 독립 하트비트(heartbeat) 기록 | Python 단일 프로세스, 기본 5초 폴링·프로세스 수명 단일 5초 하트비트 실행기(ticker), PostgreSQL 연결·문장 실행 각 2초 제한, SQLite 잠금 대기 2초 제한, 종료 3초 유예 |
