...
- 429·418은 자동 재시도하지 않고 원래 상태를 마스킹된 추적 봉투(Trace Envelope)로 반환하며 그룹 냉각(cooldown)을 적용한다. 418은 응답 헤더와 공식 JSON 본문이 나타내는 기간 중 가장 긴 값을 따르고 반복 응답이 기존 냉각을 줄이지 않는다.
