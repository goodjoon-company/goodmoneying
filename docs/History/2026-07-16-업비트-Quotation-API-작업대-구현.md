...
- 공통 거래쌍·마켓(Quote)·기준 자산(Base)은 동적 엔드포인트 필드에서 제거하고 요청 직전에 공통 선택기의 최신 값만 직렬화한다.
- 빈 페이지나 전부 중복인 캔들 페이지는 방향별 종단으로 고정하고 과거·미래 조회 버튼과 끝 상태를 표시한다.
- 400·418·429와 기타 오류는 상태별 안내를 표시하면서 원본 추적을 보존하고 `Retry-After` 기반 냉각을 적용한다.
- 추적 대화상자에 포커스 트랩(focus trap)과 닫은 뒤 원본 추적 버튼으로의 포커스 복귀를 적용한다.
