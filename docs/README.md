...
5. [UI 흐름과 사양](ui/)에서 정보 구조, 상태, 반응형, 접근성, QA gate를 확인한다.
6. [운영·배포 개발 사양](03_Operations.md)에서 운영 토폴로지(Topology), 배포 흐름, 실제 운영 반영 버전과 상태를 확인한다.
7. [개발·운영 사용 안내](사용설명서-M1-업비트-수집-운영-mvp.md)로 로컬 실행과 화면별 확인 절차를 따른다.
8. 현재 활성 개선 요청과 역사 문서 구분은 [개선 요청 상태](refinements/README.md), 변경 전후 결정은 `ADR/`, 실제 검증은 `Test/`, 인계·리스크는 `History/`, 실행 단위는 GitHub Issue에서 확인한다.
...
| UI는 어떻게 흐르고 표현되는가? | `ui/` | 정보 구조, 상태, token, 반응형, 접근성·QA | API·DB 계약 재정의 |
| 운영계에는 무엇이 어떻게 배포되어 있는가? | `03_Operations.md` | 배포 트리거, 서버 역할, 배포·검증·롤백 흐름, 시점별 운영 현황 | 제품 범위·계약 상세·구현 로그 |
