...
- 하향 웹소켓(WebSocket)의 운영자 토큰·명시적 출처 허용 목록 검증과 루프백 포트 바인딩(binding)
...
독립 리뷰 후에는 무인증 하향 연결, 교차 출처 요청, 프록시 전달 헤더 위조, Upbit의 전체 구독 교체 의미론, JSON Schema와 런타임 제어 검증 불일치, 공개·비공개 소켓 상태 결합, 재시도 소진 뒤 종료 상태 누락을 각각 실패 테스트로 고정했다. Nginx·Vite가 서버에서만 운영자 토큰을 주입하고 게이트웨이는 전달 `Host`가 아니라 명시적 출처 허용 목록만 인증 근거로 사용하도록 수정했다. 모든 구독 요청은 전체 희망 구독 스냅샷(snapshot)을 보내며 가짜 상류도 교체 의미론으로 검증한다.

최종 재리뷰에서는 활성 탭의 상태 하나만 보이는 헤더, 대화상자 내부 초점 이탈, 자동 재연결 소진 뒤 같은 세션의 제어 수용, Nginx 기본 60초 유휴 시간 제한, 전달 `Host` 기반 DNS 재바인딩 우회, `VITE_OPERATOR_TOKEN`의 개발 번들 노출을 각각 실패 테스트로 재현했다. 헤더는 공개·비공개 상태를 별도 라벨과 배지로 항상 함께 표시하고, raw 대화상자는 닫기 버튼과 키보드로 읽을 수 있는 raw 프레임 사이에서 Tab·Shift+Tab 초점이 순환하도록 수정했다. 복구 불가능한 세션은 이후 제어를 `SESSION_CLOSED`로 거부하고 새 하향 연결만 허용하며, 업비트 WebSocket 프록시는 읽기·쓰기 유휴 시간 제한을 1시간으로 늘렸다. 명시적 출처 허용 목록과 서버 측 토큰 주입을 적용하고 토큰 미설정은 전체 거부한다. 고립 화면과 실제 제품 메뉴 E2E에서 두 상태의 독립성, 양방향 초점 순환, 공개 프레임과 raw 출처를 재검증했다.
...
| `uv run pytest -q` | Pass, `334 passed, 3 skipped` | 전체 Python 회귀, 기존 TestClient 폐기 예정 경고 1건 |
...
| `npm --prefix apps/web test` | Pass, `123 tests` | 연결·프레임 분리, 공개·비공개 동시 상태, 자산·주문 구조화, 출처 표시, 탭·대화상자 초점 접근성을 포함한 전체 프런트엔드 회귀 |
| `npm --prefix apps/web run build` | Pass | TypeScript와 Vite 운영 빌드 성공 |
...
| `npx playwright test` | Pass, `14 passed` | Quotation·Exchange·WebSocket 제품 메뉴를 포함한 전체 브라우저 E2E 회귀와 시험 서버 종료 확인 |
...
리뷰에서 상향 연결 예외 원문의 URL·헤더 노출, 재연결 중 세션 종료 가능성, raw payload의 민감 키 반사, 연결별 메시지 제한기의 재연결 후 공유, 상향 정상 종료의 자동 재연결 누락, 공통 페어 선택의 상위 동기화 누락과 하향 인증 부재를 발견했다. 고정 오류 이벤트, 파싱 뒤 재직렬화 마스킹, 상향 연결마다 새 메시지 제한기, 정상 종료와 예외의 동일 재연결 흐름, 제어 가능한 공통 페어 속성, 같은 출처 역프록시의 서버 측 토큰 주입과 명시적 출처 허용 목록 회귀 테스트로 수정했다.

최종 대조에서 브라우저에는 키 입력·토큰 헤더·URL 토큰이 없고 비공개 JWT는 서버 상향 핸드셰이크에만 존재하며 `Origin`은 전달하지 않는다. 프록시는 운영자 토큰을 덮어써서 주입하고, 게이트웨이는 토큰과 명시적 출처 허용 목록을 모두 확인한다. 프로덕션 URL은 카탈로그로 고정되고 테스트 재정의는 `ws://127.0.0.1`·`localhost`·`::1`과 명시적 플래그가 모두 있어야 한다. 구독 요청과 해제는 전체 희망 구독 스냅샷으로 상향 상태를 일치시키고, 세대 번호가 이전 수신 작업의 중복 재연결을 막는다. 치명적(Critical)·중요(Important) 미해결 발견사항은 0건이다.

## 미검증 항목

- 실제 Upbit 계정·키·인터넷 상향 연결은 보안 범위에 따라 의도적으로 실행하지 않았다.
- 전체 작업대의 배포 구성·교차 기능 QA·최종 문서 인계는 Issue #24에서 완료한다.

## 결론

가짜 자격 증명과 루프백 상류만 사용한 자동화 검증에서 공개·비공개 웹소켓 중계, 공식 요청 제한, 재연결·정리, 비밀정보 경계, 제품 메뉴 시각화와 공통 페어 제어가 계약과 Issue #23 완료 조건에 일치한다.
