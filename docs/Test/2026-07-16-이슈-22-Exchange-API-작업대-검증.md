...
| 독립 리뷰 보강 표적 테스트 34개 | 비-2xx 추적 폐기, `any_of_required` 누락, 차단 미리보기 마켓 누락, `false` 생략, 탭·모달 접근성 누락으로 18개 실패 |
| 추적 봉투·차단 검증·탭 관계 엄격 테스트 | 불완전 추적 봉투 오인, 차단된 취소 기능의 조합 상태 누락, 비활성 탭 패널 대상 누락으로 3개 실패 |
...
| 명령 | 결과 |
|---|---|
| 신규 웹 단위 테스트(Unit Test) 2개 파일 | 표적 37개 통과, 1.31초 |
| 게이트웨이 상태·OpenAPI 표적 테스트 | 3개 통과, 기존 FastAPI 테스트 클라이언트 경고 1건 |
| 유효·부재·잘못된 파일 자격 증명 상태 테스트 | 3개 통과 |
| 격리 Playwright E2E | Chromium 7개 통과, 2.7초 |
| `npm test` | 14개 파일, 105개 테스트 통과, 7.99초 |
| `npm run build` | TypeScript 빌드와 Vite 프로덕션 빌드 통과, 1,835개 모듈 변환, 143밀리초 |
| `uv run pytest -q` | 289개 통과, 3개 건너뜀, 기존 FastAPI 테스트 클라이언트 경고 1건, 46.25초 |
| `uv run mypy apps/api apps/worker apps/upbit_gateway packages/shared tests` | 67개 소스 파일 오류 없음 |
| `uv run ruff check .` | `All checks passed!` |
| 이슈 전용 포트의 `npm run e2e` | 전체 Chromium E2E 12개 통과, 59.6초, API·웹 테스트 서버 종료 확인 |
...
4. 400·401·418·422·429·5xx를 사용자 메시지로 구분하면서 마스킹된 추적 봉투는 팝업으로 보존하고, 안전하지 않은 원본 오류 상세가 문서 객체 모델(DOM)에 없는지 확인한다.
5. `get-order`의 대체 필수 입력 조합을 안내하고 누락된 조회를 게이트웨이와 상향 호출 전에 차단한다. 단위 테스트는 `cancel-order`, `get-withdrawal`, `get-deposit`와 복합 `txid + currency`까지 검증한다.
6. 390px 뷰포트(viewport)에서 본문 가로 넘침이 없고 탭을 방향키로 이동하며, 추적 모달의 초기 포커스·Escape 닫기·호출 버튼 복귀를 확인한다.
...
- 계약(Contract): 카탈로그 38개와 기존 `any_of_required`를 소비하며 복사 목록을 제품 코드에 넣지 않았다. `get-order`, `cancel-order`, `get-withdrawal`, `get-deposit`의 단일·복합 대안을 실행 전에 검증한다. OpenAPI와 FastAPI `/health`에 값 없는 `credentials_configured: boolean`만 추가하고 런타임 스키마 동등성 테스트를 유지했다. DB 계약 변경은 없다.
...
- 디자인(Design): 7개 탭, 가로 스크롤 기능 목록, 2열 요청·응답 패널, 위험 빨간 배너, 테스트 초록 배지, 표·상태 카드·추적 대화상자를 사용했다. 각 탭의 `aria-controls`·탭 패널, 좌우·Home·End 방향키, 모달 초기 포커스·내부 유지·Escape·호출 버튼 복귀를 제공한다. 780px 이하 단일 열과 390px 본문 넘침을 Playwright로 검증했다.
- 운영(Operations): 서버 상태는 유효한 직접 키 쌍 또는 읽을 수 있는 유효 파일 쌍만 `설정됨`으로 표시한다. 값이나 소스 경로는 반환하지 않는다.

독립 리뷰 결과는 Critical 0건, Important 4건, Minor 2건이었고 모두 수정했다. Important는 비-2xx 추적 봉투 보존, `any_of_required` 안내·선검증, 차단 미리보기 공통 마켓, 통합 한계 명시였다. Minor는 명시적 `false` 직렬화와 탭·모달 접근성이었다. 추가 자체 리뷰에서 불완전 객체를 추적 봉투로 오인하지 않도록 구조 검증을 강화하고, 영구 차단된 `cancel-order`도 조합 충족 상태를 표시하며, 비활성 탭의 제어 대상도 문서 객체 모델에 유지했다. 공통 마켓은 제어·비제어 방식 모두 지원하고 기능 전환 후에도 보존한다. 요청 세대(generation)와 `endpoint_id`를 함께 검증해 이전 기능 또는 다른 출처의 결과·추적을 폐기한다. 공용 Product·Architecture·ADR·refinement와 카탈로그는 병렬 이슈 충돌 방지를 위해 수정하지 않았고, 현재 설계 준수 결과만 이 문서에 남겼다.

## 독립 브랜치 한계와 #24 병합 선행 조건

이 브랜치는 #21 공용 셸(shell)과 제품 메뉴를 수정하지 않으므로 `apps/web/src/features/upbitExchange/e2e.html` 격리 경로에서 가짜 게이트웨이로만 화면 E2E를 검증했다. 따라서 사용자가 실제 제품 메뉴로 진입해 HTTP 클라이언트, FastAPI 게이트웨이, 가짜 업비트 상향 서버까지 통과하는 통합 흐름은 이 브랜치만으로 증명하지 않는다.

#24 병합 단계에서는 #21 셸 연결 후 다음 통합 E2E를 병합 선행 조건으로 실행해야 한다.

1. 실제 제품 메뉴에서 Exchange 작업대로 진입한다.
2. 브라우저의 `createHttpExchangeGateway`가 시험 FastAPI의 `/health`, `/v1/catalog`, `/v1/requests`를 호출한다.
3. FastAPI가 실제 키가 아닌 가짜 자격 증명으로 가짜 업비트 상향 서버를 호출한다.
4. 조회·주문 테스트·비-2xx 추적·`blocked` 무호출과 서버 종료를 한 흐름에서 검증한다.
