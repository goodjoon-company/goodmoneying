...
| `internal-realtime-stream.schema.json` | P2 내부 WebSocket envelope, sequence, cursor, heartbeat, snapshot_required, slow_consumer 신호 | `useRealtimeAnalysis`, `realtimeStream.ts`, WebSocket 계약 테스트 |
...
- 분석 WebSocket 복구용 REST snapshot은 HTTP 경로이므로 `openapi.yaml`의 `/v1/realtime/analysis/snapshot`을 기준으로 한다.
- 전략 그래프 검증과 불변 전략 버전 REST 경계는 `openapi.yaml`의 `/v1/strategy-graphs/validate`, `/v1/strategies`, `/v1/strategies/{strategyId}/versions`, `/v1/strategy-versions/{strategyVersionId}`를 기준으로 한다.
