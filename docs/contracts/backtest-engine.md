...
이 문서는 P4 백테스트 엔진의 순수 도메인 경계(pure domain boundary)를 정의한다. P4-1은 `packages/shared/goodmoneying_shared/backtest_engine.py`의 결정론적 캔들 재생 엔진, P4-2는 `backtest_runs` 계열 영속화, P4-3은 저장된 run을 읽는 REST와 Backtest Lab 조회 화면까지 구현했다. Worker 실행 접수, 목록 pagination, WebSocket 진행 이벤트는 후속 조각이다.
...
- `GET /v1/backtest-runs/{backtestRunId}` 조회 API와 Backtest Lab 읽기 전용 화면
- Backtest Worker 임대·재시도·산출물 저장
- REST 목록·대용량 결과 pagination, WebSocket 진행 이벤트
