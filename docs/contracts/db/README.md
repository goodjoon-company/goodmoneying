...
- 연결 역할은 마이그레이션이 요구하는 DDL(Data Definition Language)과 데이터 변경 권한을 가져야 한다. 기준선 마이그레이션은 연결 URL이 선택한 `current_database()`의 기본 시간대를 KST로 설정하므로 DB 소유자(database owner) 또는 슈퍼유저 권한이 필요하다.
