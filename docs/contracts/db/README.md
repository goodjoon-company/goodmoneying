...
- P5-4 risk evaluation은 새 migration 없이 P5-1의 위험·주문·kill switch 계약과 P5-3의 paper execution queue 계약을 소비한다. `created` 주문 의도는 위험 평가 transaction에서 `approved|risk_rejected`로 전이하고, 승인된 paper 주문만 `paper_execution_jobs`에 연결한다.
- `20260718000800_p5_reconciliation_runs.sql`은 P5 paper/shadow 내부 대사 실행 증적을 추가하는 forward-only 계약이다. 실제 private 계좌 조회는 P6 이후 범위이며, P5-5는 `exchange_orders`, `order_fills`, `position_projections`, `risk_events`의 내부 원장 일관성을 검증한다.
- `20260718000900_p6_order_identity_separation.sql`은 P6 order-test 증적과 live 주문 identifier를 분리한다. `upbit_order_test_runs`의 응답 UUID·identifier는 조회·취소 불가 append-only 증적으로만 보존하고, `live_order_identifiers`는 `gm1_` 기반 deterministic identifier와 `order_intents`를 별도 매핑한다. `upbit_order_identifier_reservations`의 `(exchange_account_id, identifier)` unique key와 trigger가 `order_intents.idempotency_key`에서 벗어난 live identifier, order-test 응답 식별자의 live 재사용, live/test 동시 재사용 경쟁을 거부한다.
- `20260718001000_p6_live_capability_gate.sql`은 P6 live capability 권위 상태를 append-only로 추가한다. `trading_capabilities`의 최신 global 행만 평가 대상이며, CI·AI·service actor 기록과 증적 UPDATE·DELETE를 DB에서 거부한다.
- `20260718001100_p6_safe_order_outbox.sql`은 주문하기·주문조회 권한이 있고 출금 권한이 없는 API Key 준비도 증적과 실제 제출 전 안전 주문 outbox를 추가한다. `ready` outbox는 승인 완료 주문 의도, live identifier, 권한 증적의 계좌·주문 일치를 DB에서 강제하며 `submit_attempt_count=0`으로 실제 제출 시도를 막는다.
- `20260718001200_p6_live_order_binding.sql`은 Upbit live 주문 UUID·identifier와 내부 `exchange_orders`를 결합하는 append-only 증적을 추가한다. live `exchange_orders`는 같은 transaction 안에서 `upbit_live_exchange_order_bindings`와 결합되어야 커밋되며, order-test 응답 UUID·identifier를 live 결합에 사용할 수 없다.
- `20260718001300_p6_order_submit_rehearsal.sql`은 실제 `POST /v1/orders` 제출 전 dry-run/rehearsal 증적을 추가한다. `upbit_order_submit_rehearsals`는 ready outbox, reserved live identifier, 만료되지 않은 permission attestation, 기존 live binding 부재를 DB에서 강제하며 `actual_request_sent=false`, `would_submit=false`, `can_bind_response=false`, 응답 UUID·identifier 없음만 허용한다.
- `20260718001400_p6_live_reconciliation_application.sql`은 결합된 Upbit live 주문에 terminal REST snapshot 대사를 적용한 append-only 증적을 추가한다. `upbit_live_reconciliation_applications`는 live binding, `reconciliation_runs`, REST snapshot evidence의 UUID·identifier·state·source endpoint 일치를 DB에서 강제하며 `can_resubmit=false`, `actual_request_sent=false`, `actual_order_cancel_sent=false`만 허용한다.
