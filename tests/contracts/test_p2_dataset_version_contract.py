...
def test_P2_6_생성작업목록은_새로고침_재발견용_cursor와_응답을_정의한다() -> None:
    document = yaml.safe_load(OPENAPI.read_text())
    operation = document["paths"]["/v1/dataset-builds"]["get"]
    parameters = {item["name"]: item for item in operation["parameters"]}
    ...
    assert "생성 작업 ID 상한" in parameters["cursor"]["description"]
    response = operation["responses"]["200"]["content"]["application/json"]["schema"]
    assert response["$ref"] == "#/components/schemas/DatasetBuilds"
    builds = document["components"]["schemas"]["DatasetBuilds"]
    assert builds["required"] == ["items", "nextCursor"]
    assert builds["properties"]["nextCursor"]["type"] == ["string", "null"]


def test_P2_6_Data_Lab_시장선택은_data_foundation_instrumentId를_사용한다() -> None:
    document = yaml.safe_load(OPENAPI.read_text())
    market = document["components"]["schemas"]["DataFoundationMarket"]

    assert "instrumentId" in market["required"]
    assert market["properties"]["instrumentId"] == {
        "type": "integer",
        "format": "int64",
        "minimum": 1,
    }
