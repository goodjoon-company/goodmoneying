...
def test_live_postgres_백테스트_run과_결과는_멱등_불변으로_저장된다() -> None:
    ...
    trade_page = store.list_run_trades(
        backtest_run_id=int(saved["backtestRunId"]),
        page_size=1,
        cursor=None,
    )
    equity_page = store.list_run_equity_points(
        backtest_run_id=int(saved["backtestRunId"]),
        page_size=1,
        cursor=None,
    )
    assert trade_page is not None
    assert equity_page is not None
    assert trade_page["items"][0]["remainingQuantity"] == Decimal("2.00")
    assert equity_page["items"][0]["equity"] == Decimal("1009.579790")
