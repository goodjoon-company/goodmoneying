...
test("공식 주문 테스트는 확인 대화상자 없이 가짜 상향에 한 번만 전송한다", async ({ page }) => {
  ...
  await page.getByLabel("market").fill("KRW-BTC");
  await page.getByLabel("side").selectOption("bid");
  await page.getByLabel("price").fill("1000");
  await page.getByLabel("ord_type").selectOption("price");
...
test("390px에서 본문 가로 넘침 없이 키보드로 탭과 기능을 탐색한다", async ({ page }) => {
  ...
  await expect(page.getByRole("tab", { name: /주문/ })).toHaveAttribute("aria-selected", "true");
