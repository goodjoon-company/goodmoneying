...
test("고립된 업비트 웹소켓 작업대가 공개·비공개 제어와 raw 추적을 수행한다", async ({ page }) => {
  ...
  await expect(workbench.getByLabel("공개 연결 상태", { exact: true })).toHaveText("connected");
  await expect(workbench.getByLabel("비공개 연결 상태", { exact: true })).toHaveText("closed");
  ...
  const rawDialog = page.getByRole("dialog", { name: "raw frame 추적" });
  const closeRaw = page.getByRole("button", { name: "닫기" });
  await expect(rawDialog).toContainText("browser-trace");
  await expect(rawDialog).toContainText("2026.07.16 09:00:00 KST");
  await expect(closeRaw).toBeFocused();
  await page.keyboard.press("Shift+Tab");
  await expect(rawDialog.getByLabel("raw frame 1")).toBeFocused();
  await page.keyboard.press("Tab");
  await expect(closeRaw).toBeFocused();
  await closeRaw.click();

  await page.getByRole("tab", { name: "내 자산" }).click();
  await page.getByRole("button", { name: "연결", exact: true }).click();
  await expect(workbench.getByLabel("공개 연결 상태", { exact: true })).toHaveText("connected");
  await expect(workbench.getByLabel("비공개 연결 상태", { exact: true })).toHaveText("connected");
  await page.getByRole("button", { name: "구독", exact: true }).click();
