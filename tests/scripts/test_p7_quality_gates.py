...
def test_P7_release_gate는_계획상태_gate가_남아있으면_실패한다() -> None:
    manifest = load_manifest(ROOT / "docs/contracts/quality/p7-quality-evidence.yaml")
    mutated = {
        **manifest,
        "gates": [
            {**gate, "status": "planned"} if gate["id"] == "hygiene.unresolved_artifacts" else gate
            for gate in manifest["gates"]
        ],
    }

    with pytest.raises(QualityGateError, match="P7 release gate 미통과"):
        validate_release_manifest(mutated, root=ROOT)
