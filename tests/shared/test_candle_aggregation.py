...
def test_집계_워커는_자동_작업을_완료하고_진행률을_100으로_갱신한다() -> None:
    ...
    job = repository.candle_rollup_recompute_job(1)
...
def test_집계_워커는_첫_처리_구간이_32초_걸려도_31초_시점에_동작_중이다(
    ...
    original_materialize = repository.run_candle_rollup_recompute_job
    ...
    monkeypatch.setattr(repository, "run_candle_rollup_recompute_job", delayed_materialize)
    results: list[int] = []
...
def test_집계_실패_후에는_heartbeat_ticker_스레드가_남지_않는다(
    ...
    monkeypatch.setattr(repository, "run_candle_rollup_recompute_job", fail_materialize)
...
def test_sqlite_집계는_heartbeat가_교차된_뒤_실패해도_부분_커밋하지_않는다(
    ...
    def attempt_heartbeat_after_first_rollup_write() -> None:
        if not first_rollup_written.wait(timeout=1):
            return
    ...
    heartbeat_thread = threading.Thread(
        target=attempt_heartbeat_after_first_rollup_write,
        name="candle-aggregation-heartbeat-test",
    )
    heartbeat_thread.start()

    with pytest.raises(RuntimeError, match="두 번째 집계 쓰기 실패"):
