...
def test_범위_재계산은_영향_버킷만_바꾸고_성공_상태를_기록한다() -> None:
    ...
    untouched_before = repository._execute(
        ...
        ((start + timedelta(minutes=3)).astimezone(KST).isoformat(),),
    ).fetchone()
...
def test_집계_개정은_과거를_보존하고_현재와_시점_기준_투영을_선택한다() -> None:
    ...
    revisions = repository._execute(
        ...
        ORDER BY id
        """,
        (instrument_id, start.astimezone(KST).isoformat()),
    ).fetchall()
...
def test_영향_범위_밖_커버리지는_무효화_입력_해시를_바꾸지_않는다() -> None:
    ...
    repository._execute(
        ...
        (
            instrument_id,
            outside_start.astimezone(KST).isoformat(),
            (outside_start + timedelta(minutes=1)).astimezone(KST).isoformat(),
        ),
    )
    changed = SourceCandle(
        **{
...
def test_부분_커버리지_전이는_잔여_구간과_전체_영향_버킷_스냅샷을_보존한다() -> None:
    ...
    repository._execute(
        ...
        (
            instrument_id,
            start.astimezone(KST).isoformat(),
            (start + timedelta(minutes=3)).astimezone(KST).isoformat(),
        ),
    )
