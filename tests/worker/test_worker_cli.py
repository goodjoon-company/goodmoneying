...
def test_worker_repository_requires_explicit_runtime_mode(
    ...
    with pytest.raises(RuntimeError, match="RUNTIME_MODE"):
        runtime.create_repository_from_environment()
    with pytest.raises(RuntimeError, match="RUNTIME_MODE"):
...
def test_backfill_collection_worker_reports_gated_without_consuming_job(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []

    class GatedRepository(FakeRepository):
        def backfill_claim_gate_reason(self) -> str:
            return "운영 승인 SHA 불일치"

    class GatedWorker:
        def __init__(self) -> None:
            self.repository = GatedRepository(calls)

        def run_backfill_once(self, on_progress: Callable[[], object] | None = None) -> int:
            pytest.fail("안전 게이트가 닫히면 claim 경로를 호출하면 안 된다.")

    def stop_after_report(_seconds: float) -> None:
        calls.append("sleep")
        raise KeyboardInterrupt

    monkeypatch.setattr(
        "goodmoneying_worker.backfill_collection_worker.time.sleep", stop_after_report
    )

    backfill_collection_worker.run_backfill_poll_loop(GatedWorker(), poll_seconds=10)  # type: ignore[arg-type]

    assert calls == ["heartbeat:backfill_collection:gated", "sleep"]
...
def test_집계_워커는_하트비트를_기록하고_기본_5초_주기로_실행한다(
    ...
    class FakeAggregationWorker:
        ...
        def record_heartbeat(self, status: str, error_message: str | None = None) -> None:
...
def test_집계_heartbeat_저장소는_postgres와_sqlite별_2초_제한을_사용한다(
    ...
    source_repository = SQLiteOperationsRepository.from_path(tmp_path / "heartbeat.sqlite3")
    sqlite_repository = runtime.create_heartbeat_repository_from_environment(source_repository)
...
def test_차단된_heartbeat_중_집계가_3회_실패해도_소유자는_유한_시간에_반환한다(
    ...
    assert len(blocked_threads) <= 1
    assert any(
        message.startswith("aggregation_failed_heartbeat_skipped") for message in caplog.messages
    )
...
def test_heartbeat가_차단되지_않으면_집계_실패_상태를_저장한다() -> None:
    ...
    runtime_status = heartbeat_repository.collection_worker_runtime_status("candle_aggregation")
